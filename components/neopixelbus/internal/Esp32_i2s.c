...
// #include "esp_intr.h"	idf-port
#include "esp_intr_alloc.h"	// esp_intr.h is deprecated, please include esp_intr_alloc.h instead

//#include "rom/ets_sys.h"	// idf-port
#include "esp32/rom/ets_sys.h"	// rom/ets_sys.h is deprecated, please use esp32/rom/ets_sys.h instead
...
#include "Esp32_i2s.h"
// #include "esp32-hal.h"		// idf-port
#include "esp_log.h"			// added for idf-port ( _LOGE )
...
void i2sSetPins(uint8_t bus_num, int8_t out, int8_t ws, int8_t bck, int8_t in, bool invert) {
    ...
    if (ws >= 0) {
        if (I2S[bus_num].ws != ws) {
			...
			// pinMode(ws, OUTPUT);	// idf-port

			// begin inserted idf-port
...
//    double mclk;
    ...
    if (clkmdiv > 256) {
		ESP_LOGE("NEOPIXEL", "rate is too low");
        return ESP_FAIL;
    }
...
size_t i2sWrite(uint8_t bus_num, uint8_t* data, size_t len, bool copy, bool free_when_sent) {
    ...
    while (len) {
        ...
        if (xQueueReceive(I2S[bus_num].tx_queue, &item, portMAX_DELAY) == pdFALSE) {
			ESP_LOGE("NEOPIXEL", "xQueueReceive failed\n");
            break;
        }
